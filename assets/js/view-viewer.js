...
export function renderViewer(state, callbacks) {
  ...
  if (Array.isArray(state.currentProjectMeta?.gallery) && state.currentProjectMeta.gallery.length > 0) {
    ...
    headerActions.appendChild(slideBtn);
  }
  if (isParentMode) {
  ...
  if (steps.length === 0) {
    ...
    if (visibleChildren.length > 0) {
      ...
      visibleChildren.forEach(child => {
        ...
        loadProjectMeta(`${state.currentPath}/${child.path}`).then((childMeta) => {
          if (Array.isArray(childMeta?.gallery) && childMeta.gallery.length > 0) {
            const slideBtn = document.createElement('button');
            slideBtn.type = 'button';
            ...
            `;
            slideBtn.style.top = 'var(--spacing-md)';
            slideBtn.style.right = 'var(--spacing-md)';
            slideBtn.addEventListener('click', (e) => {
              e.stopPropagation();
              window.location.hash = `#/slideshow/${state.currentPath}/${child.path}`;
            });
